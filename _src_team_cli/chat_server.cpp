...
public:
    ...
    */
    void getBet()
    {
        bool notAnumber = false; // TODO check if bet is a positive number
        std::cout << "GET BET" << std::endl;
        //int bet = read_msg_.ca.bet; // transfer bet to this
        //originalPlayerHand_.placeBet(bet);
    }
    ...
    asio::async_read(socket_,
        ...
        [this, self](std::error_code ec, std::size_t /*length*/)
        {
            if (!ec)
            {
                ...
                if(read_msg_.ca.join && read_msg_.ca.name_valid) // fix so that one client doesn't start the game
                {
                ...
                if(room_.all_players_have_a_name())
                {
